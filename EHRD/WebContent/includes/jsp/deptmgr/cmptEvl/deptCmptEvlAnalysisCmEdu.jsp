...
yepnope([{
    ...
    complete: function() {
        ...
        var recommEduDataSource = new kendo.data.DataSource({
            type: "json",
            transport: {
                read: { url:"/deptmgr/ca/dept_cmpt_evl_recomm_edu_list.do?output=json", type:"POST" },
                parameterMap: function (options, operation){    
                    return {
                        RUN_NUM: runNum,
                        TG_USERID: tgUserid
...
function viewEdu(subjectNum){
    ...
    $.ajax({
        type : 'POST',
        url: "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/deptmgr/ca/dept_cmpt_evl_recomm_edu_detail_info.do?output=json",
...
function fn_link(subjectNum){
	...
	$.ajax({
        type : 'POST',
        url: "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/deptmgr/ca/dept_cmpt_evl_recomm_edu_url_info.do?output=json",
        data : { SUBJECT_NUM : subjectNum },
        complete : function( response ){
            if(response.responseText){
                var obj  = eval("(" + response.responseText + ")");
        ...
        <div class="cont_body">
            ...
            <div id="graph_list">
                ...
                <div id="example" class="k-content" style="width: 1062px; height: 100px;">
				    <div id="cmptgrid" data-bind="source: gridRows" style="width: 1062px; height: 90px;"></div>
				</div>
            ...
            <div id="pop1-window" style="display:none;">
                <div class="layer_cont">
                    <div class="layer_text">
                        <div class="sub_title"><span id="eduTitle"></span></div>
                        <div class="sub_btn"><a href="javascript:fn_link();"><img src="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/btn/btn_go01.gif" alt="바로가기"/></a></div>
