            ...
            if(view.length>0){
                ...
                for(var i=0; i<view.length; i++){
                    ...
                    columns.push({
                        ...
                        headerAttributes:{"class":"table-header-cell", style:"text-align:center"},
                        attributes:{"class":"table-cell", style:"text-align:center"},
                        template:"<button onclick=\"filterData("+item.CMPNUMBER+"); return false;\" class=\"k-button\" >"+entryValue+"점</button>"//" <a href='javascript:void();' onclick='javascript:filterData("+item.CMPNUMBER+");' ><ins>"+entryValue+"</ins></a>"
                    });
...
//교육정보 바로가기 링크
function fn_link(subjectNum){
	if(subjectNum==null || subjectNum==""){
		subjectNum = $("#SUBJECT_NUM").val();
	}
	
	$.ajax({
        type : 'POST',
        url: "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/cam/cmpt_evl_recomm_edu_url_info.do?output=json",
        data : { SUBJECT_NUM : subjectNum },
        complete : function( response ){
            if(response.responseText){
                var obj  = eval("(" + response.responseText + ")");
                if(obj!=null){
                    if(obj.statement != "NO"){
                        //새창 띄우기..
                        window.open(obj.statement, "_blank");
                    }else{
                        alert("바로가기 URL정보가 존재하지 않습니다.");
                    }
                }
            }
        },
             ...
             <div class="sub_cont">
                    ...
                    <div id="graph_list">
                        ...
                        <div style="height:35px;">
                            <button onclick="filterData(); return false;" class="k-button" >전체보기</button>  ※ 역량별 취득점수를 클릭하면 추천교육이 조회됩니다.
                        </div>
