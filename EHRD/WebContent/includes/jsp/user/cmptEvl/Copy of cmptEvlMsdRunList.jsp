...
<script type="text/javascript">	
    ...
    function openwindow() {
        ...
        $("#grid").kendoGrid({
               dataSource: {
                    type: "json",
                    transport: {
                        read: { url:"<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/ca/cmpt_evl_msd_run_list.do?output=json", type:"POST" },
                        	...
                        	if(dataItem.SELF_CMPL_FLAG == "Y"){
                                if(dataItem.ONE_START_DATE == null || dataItem.ONE_START_DATE == ""){
                                    ...
                                    if(dataItem.EVL_CMD == "평가하기"){
                                    	if(dataItem.USERID_EXED_CNT=="1"){
                                    		...
                                    		return "<input type='button' class='k-button' value='재평가하기' onclick='fn_evlMsdExec("+dataItem.RUN_NUM+","+dataItem.TG_USERID+");'/>"+"<br>"+dataItem.SELF_CMPL_DATE;
                                    	}
                                ...
                                if(dataItem.EVL_CMD == "평가하기"){
                                	if(dataItem.USERID_EXED_CNT=="1"){
                                		...
                                		return "<input type='button' class='k-button' value='평가하기' onclick='fn_evlMsdExec("+dataItem.RUN_NUM+","+dataItem.TG_USERID+");'/>";
                                	}
    ...
    function fn_evlExec(runDiv, runNum, tgUserid){
    	...
    	if(isConfirm){
    		...
    		$("#f_page").val("R"); //평가 완료 후 포워딩될 페이지 값..
            
    		document.frm.action="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/ca/cmpt_evl_msd_self_exec.do";
    		document.frm.submit();
        }
    ...
    // 다면역량평가 하기.
    function fn_evlMsdExec(runNum, tgUserid){
        //alert(runNum);
        $("#RUN_NUM").val(runNum);
        $("#TG_USERID").val(tgUserid);
