...
var tgUserid = "<%=action.getUser().getUserId()%>";
var job = "<%=jobNum%>";
...
yepnope([{
	...
	complete: function() {
        ...
        $("#printBtn").click(function(){
            window.open('<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/cam/cmpt_evl_print.do?RUN_NUM='+$("#RUN_NUM").val()+'&TG_USERID='+$("#TG_USERID").val()+'&JOB='+$("#JOB").val()+'&LEADERSHIP='+$("#LEADERSHIP").val(), '', 'scrollbars=yes, width=740, height=500');
        });
	}
}]);       
...
function goPage(pageNum) {
    if(pageNum==1){
    	...
    	$.ajax({
            ...
            complete : function( response ){
                ...
                if(obj.totalItemCount < 2){
                    ...
                    document.frm.action = "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/cam/cmpt_evl_analysis_cmpt_grow.do";
                    document.frm.submit();
                }                           
            },
    	...
    	//추천교육
    	document.frm.action = "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/cam/cmpt_evl_analysis_cm_edu.do";
    	document.frm.submit();
    }else if(pageNum==4){
    	//코멘트
    	document.frm.action = "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/cam/cmpt_evl_analysis_user_cmt.do";
    	document.frm.submit();
    }
...
<body>
	<form id="frm" name="frm"  method="post" >
	    ...
	    <input type="hidden" name="TG_USERID" id="TG_USERID" value="<%=action.getUser().getUserId()%>"/>
        ...
        <div id="cont_body">
         <div class="content">
             ...
             <div class="sub_cont">
                    ...
                    <div class="tab_box" id="cmptAnalyCmt">
                        ...
                        <span style="background:url(<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 진단자 역량수준 비교결과 부족 역량: <u><span id="r_weak_cmpt"></span></u><br>
                        <span style="background:url(<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 전사 직원대비 부족 역량: <u><span id="a_weak_cmpt"></span></u><br>
                        <span style="background:url(<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 동일 계층 직원대비 부족역량: <u><span id="l_weak_cmpt"></span></u><br>
                        <span style="background:url(<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 동일 직무 직원대비 부족역량: <u><span id="j_weak_cmpt"></span></u>
                    </div>
                        ...
                        <ul id="graph_tab">
                            ...
                            <li><a href="#graph3" class="tab"><img src="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/btn/btn_te.gif" alt="테이블차트" /></a>
                                <div id="graph3" class="line" >
                                    <!-- 테이블차트 -->
                                     <div id="grid" style="width:1160px;"></div>
                                </div>
                            </li>
                        </ul>
                        <script type="text/javascript">
