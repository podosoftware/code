...
yepnope([{
    ...
    complete: function() {
      ...
      function runFilter(){
        ...
        //생성 detail 그리기
        $('.detail_Info').show().html(kendo.template($('#cre_detail_template').html()));
		...
		$("#grid").kendoGrid({
            dataSource: {
                type: "json",
                transport: {
                    read: { url:"<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/mtr/mtr_run_app_list.do?output=json", type:"POST" },
                    parameterMap: function (options, operation){	
                    	var sortField = "";
                    	var sortDir = "";
            ...
            columns: [
				...
				},
				{
				    ...
				    template: function (data) {
                    	if(data.MTR_REQ_DIV_CD=="4"){
                            	return "생성";
                    	}else if(data.MTR_REQ_DIV_CD=="5"){
                    		return "완료";
                    	}else{
                        	return "";
                    	}
...
function fn_detailView(mtrSeq){
    ...
    $("#lineSeq").val(res[0].REQ_LINE_SEQ);
    $("#lastLineSeq").val(res[0].LAST_REQ_LINE_SEQ);
    //파일 업로드 다운로드를 위한 value
    $("#mentorId").val(mtrSeq);
    $("#menteeId").val(res[0].MENTEE_SEQ);
    $("#adminYn").val(res[0].HRD_ADMIN_YN);
   ...
   if(res[0].REQ_STS_CD == "2"){ //맨토링이 승인 되면 버튼 숨김
	   $("#mtrComp").hide();
	   $("#mtrClose").hide();
	   $("#insertReq").hide();
	   $("#insertClose").hide();
...
//승인처리
 function apprReq(){
	if($("#adminYn").val()=="Y"){
			alert("최종 승인자가 승인한 멘토링입니다.");
			return;
	}		
	 var isDel = confirm("해당 멘토링을 승인하시겠습니까?");
		if(isDel){
		 //승인요청
		    $.ajax({
		       type : 'POST',
		       url: "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/mtr/mtr_app_save.do?output=json",
		       data : {
		    	   tu:$("#tu").val(),
	    	   	   MTR_SEQ : $("#mtrSeq").val(),
		    	   REQ_NUM : $("#reqNum").val(),
		    	   REQ_LINE_SEQ : $("#lineSeq").val(),
		    	   LAST_REQ_LINE_SEQ :$("#lastLineSeq").val()
		       },
 ...
 //미승인 처리
 function notApprReq(){
		var isDel = confirm("해당 멘토링을 미승인하시겠습니까?");
		if(isDel){
		 //미승인요청
		    $.ajax({
		       type : 'POST',
		       url: "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/mtr/mtr_not_app_save.do?output=json",
		       data : {
		    	   tu:$("#tu").val(),
	    	   	   MTR_SEQ : $("#mtrSeq").val(),
		    	   REQ_NUM : $("#reqNum").val(),
...
function deleteMtr(){
	...
	var isOperator = <%=isSystem%>;
	var isManager = <%=isSystem%>;
	
	if(isSystem==true || isOperator==true || isManager==true){
		var isDel = confirm("멘토링 삭제 하시겠습니까? 삭제 시 해당 데이터는 조회 할수 없습니다.");
		if(isDel){
			$.ajax({
			       type : 'POST',
			       url: "<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/mtr/delete_my_mtr.do?output=json",
			       data : { 
			    	   MTR_SEQ : $("#mtrSeq").val()
			       },
			       complete : function( response ){
		...
		alert("부서장 이상의 권한을 가져야 삭제가 가능합니다.");
	}
}
...
<body>
    <form id="frm" name="frm"  method="post" >
        ...
        <input type="hidden" name="lineSeq" id="lineSeq"/>
        <input type="hidden" name="lastLineSeq" id="lastLineSeq" />	
	...
	<script type="text/x-kendo-template"  id="cre_detail_template"> 
		...
		<div class="dl_wrap">
			<dl>
				...
				<dd class="fir inp_top">
					<input id="mentorNm" type="text" class="k-textbox inp_style02" disabled data-bind="value:MTR_NM" style="width:270px;" title="멘토링명 입력"/>
				</dd>
			</dl>
			<dl>
			...
			<dl>
				...
				<dd>
					<input type="text" id="mentorText" class="k-textbox inp_style02" disabled data-bind="value:MENTOR_NM" style="width:270px;margin-right:1px;" title="멘토명 입력"/>
				</dd>
			</dl>
			<dl>
				<dt> 멘티 <img src="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/ico/ico_vital.gif"  alt="필수"/></dt>
				<dd id="menteeArea" style="padding-bottom:100px;">				
					<div id="menteeList"></div>
				</dd>
			</dl >
			<div class="btn_btm">
				<button id ="insertReq" class="k-button " style="width:75px;" onclick="apprReq();">승인</button>
				<button id ="insertClose" class="k-button " style="width:75px;" onclick="notApprReq();">미승인</button>
				<button class="k-button" style="width:75px;" id="mtrDelete" onclick="deleteMtr();">삭제</button>
			</div>
		</div>
</script>
...
<div class="tit">멘토링 완료 </div>
	<div class="dl_wrap">
		...
		<dl>
			<dt style = "line-height:35px;"> 멘토링기간</dt>
			<dd style = "line-height:35px;">
				<input type="text"  id="mtrStart" disabled data-bind="value:MTR_ST_DT" style="width:125px;" title="멘토링 기간 시작"/> ~ 
					<input type="text" id="mtrEnd" disabled data-bind="value:MTR_ED_DT" style="width:125px;" title="멘토링 기간 끝"/>
			</dd>
		</dl>
		<dl>
			<dt> 멘토 / 멘티</dt>
		...
		<dl>
			<dt > 훈련보고서</dt>
			<dd id="fileArea" style="padding-bottom:100px;">
				<div class="table_wp03">
		...
		<div class="btn_btm2">
					<button class="k-button" style="width:75px;" id="mtrComp" onclick="apprReq();">승인</button>
					<button class="k-button" style="width:75px;" id="mtrClose" onclick="notApprReq();">미승인</button>
					<button class="k-button" style="width:75px;" id="mtrDelete" onclick="deleteMtr();">삭제</button>
				</div>
