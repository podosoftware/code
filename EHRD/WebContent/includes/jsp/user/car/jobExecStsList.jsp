    ...
    complete: function() {
        ...
        var grid = $("#grid").kendoGrid({
            dataSource: {
                type: "json",
                transport: {
                    read: { url:"<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/car/get_job_exec_sts_list.do?output=json", type:"POST" },
                    parameterMap: function (options, operation){ 
...
function excelDownload(button){
   ...
   button.href="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/car/job_exec_sts_excel_list.do?runNum="+$("#runList").val()+"&RUN_NAME="+ $("#runList").data("kendoDropDownList").text();
}
...
<body>
    ...
    <div class="container">
        <div id="cont_body">
         <div class="content">
            <div class="top_cont">
                <h3 class="tit01">직무별응답현황</h3>
                <div class="point">
                    ※ 직무별 진단대상자수, 응답자, 미응답자 및 응답률을 볼 수 있습니다.<br/>※ 상세 정보는 진단실시현황 메뉴에서 볼 수 있습니다. <span><a href="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/service/car/assm_exct_sts_pg.do"> [바로가기]</a></span>
                </div>
                <div class="location">
                    <span><img src="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/images/ico/ico_home.png" /> &nbsp;&#62;</span>
                    <span>역량진단결과&nbsp; &#62;</span>
                    <span class="h">직무별응답현황</span>
                </div><!--//location-->
            </div>
             <div class="sub_cont">
