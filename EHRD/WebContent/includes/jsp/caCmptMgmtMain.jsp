            ...
            var dataSource = new kendo.data.DataSource({
             ...
             serverSorting: false});
              ...
              dataSource_cmpgroup = new kendo.data.HierarchicalDataSource({
                  type: "json",
                  transport: {
                      read: { url:"<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/admin/ca/ca_cmpgroup_list.do?output=json", type:"POST" },
                      parameterMap: function (options, operation){  
...
//               });
              ...
              //엑셀 업로드 팝업에서 역량군 코드를 참조할 수 있도록 제공되는 treeveiw 정의
              $("#cmpgroupRefTreeview").kendoTreeView({
                  dataTextField: "TEXT",
                  dataSource: {
                      type: "json",
                      transport: {
              ...
              $("#cmpgroup-save-btn").click( function(){
		            ...
		            if($("#mgmt-standardcode").val() == "C103"){
		               ...
		               if($("#mgmt-p_cmpGroup").val() ==""){
		                   ...
		                   for(var i = 0; i<data.length; i++) {
		                       var dataItem = data[i];
		                       if(dataItem.TEXT == $("#mgmt-cmpGroupName").val()){
		                      	if(dataItem.USEFLAG == $(':radio[id="mgmt-cmpGroupUseFlag"]:checked').val()){
		                       	
		                     	alert("사용역량군중 동일한 역량군명이 있습니다.");
		   		                return false;
		                   ...
		                   dataSource_s.filter({field : "P_VALUE", operator: "eq", value: $("#mgmt-p_cmpGroup").val() });
		                   ...
		                   for(var i = 0; i<data_s.length; i++) {
		                      ...
		                      if(dataItem.P_VALUE == $("#mgmt-p_cmpGroup").val()){
		                     	if(dataItem.USEFLAG == $(':radio[id="mgmt-cmpGroupUseFlag"]:checked').val()){
		 	                      if(dataItem.TEXT == $("#mgmt-cmpGroupName").val()){
		            ...
		            if(isSave){
		                ...
		                $.ajax({
		                    type : 'POST',
		                    url:"/admin/ca/ca_cmpgroup_save.do?output=json",
        ...
        for(var i = 0; i<data.length; i++) {
            ...
            if(cmpNumber == dataItem.CMPNUMBER){
                ...
                $("#bhvGrid").kendoGrid({
						dataSource: {
							transport: { 
								read: { url:'/admin/ca/ca_cmtp_bhv_list.do?output=json', type:'post' },
						 		parameterMap: function (options, operation){			
						...
						columns: [
								{ title: "행동지표",   field: "BHV_INDICATOR", width: 300 ,
        	   ...
        	   $("#cmpGroup_s").kendoComboBox({
                ...
                //그리드 클릭 시 역량군(대)는 data binding으로 자동 selected 되지만 역량군(소)는 역량군(대)의 값으로 filter되고나서 selected 되어야하기때문에 한번더 filter를 해준다.
                if($("#cmpGroup").val()!=""){
               	 dataSource_s.filter({field : "P_VALUE", operator: "", value: $("#cmpGroup").val() });
                }
		...
		// dtl save btn add click event
       	$("#saveBtn").click( function(){
            ...
            if(isDel){
        		var params = {
        			CMPNUMBER : $("#cmpNumber").val(),
        			CMPGROUP : $("#cmpGroup").val(),
        			CMPGROUP_S : $("#cmpGroup_s").val(),
                     CMPNAME : $("#cmpName").val(),
        		...
        		$.ajax({
        			type : 'POST',
					url:"/admin/ca/ca_cmpt_save.do?output=json",
					data : { item: kendo.stringify( params ) },
					complete : function( response ){
						var obj  = eval("(" + response.responseText + ")");
						if(obj.saveCount != 0){
    	    ...
    	    $("#grid").empty();
	       	$("#grid").kendoGrid({
	               dataSource: {
	                   type: "json",
	                   transport: {
	                       read: { url:"/admin/ca/ca_cmpt_list.do?output=json", type:"POST" },
	                       parameterMap: function (options, operation){	
	                       	return {   };
	                       } 		
	                   },
...
<body>
    ...
    <div id="content">
        <div class="cont_body">
            ...
            <div class="table_zone" >
                <div class="table_btn">
                        <button id="cmpgroupBtn" class="k-button" >역량군관리</button>&nbsp;
                        <a class="k-button"  href="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/admin/ca/ca_common_excel.do" >엑셀 코드모음</a>&nbsp;
                        <a class="k-button"  href="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/admin/ca/ca_cmpt_list_excel.do" >엑셀 다운로드</a>&nbsp;
	...
	<script type="text/x-kendo-template" id="template"> 
			<table class="tabular" id="tabular" width="100%" cellspacing="0" cellpadding="0">
			    	...
			    	<tr>
                        ...
                        <!--<td>:<input  id="cmpGroup_s" data-bind="value:CMPGROUP_S" style="border:none" /></td>-->
                    ...
                    <tr>
				    	...
				    	<td class="subject"><input type="text" class="k-textbox" id="cmpName" data-bind="value:CMPNAME" onKeyUp="chkNull(this);" style="width:97%;"  /></td>
			    	</tr>
			    	<tr>
				    	<td width="100px" class="subject"><span style="background:url(/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 역량정의</td>
				    	<td class="subject"><textarea cols="40" rows="4" type="text" class="k-textbox" id="cmpDefinition" data-bind="value:CMPDEFINITION" style="width:97%;" ></textarea></td>
			    	</tr>
					<!--<tr>
				    	<td width="80px" ><span style="background:url(/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 기업요구수준</td>
						<td><input   id="bsnsReqrLevel" data-bind="value:BSNS_REQR_LEVEL"  value="0" /></td>
			    	</tr>-->
					<tr id="bhvTR">
				    	<td width="100px" class="subject"><span style="background:url(/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 행동지표</td>
				    	<td class="subject"><div id="bhvGrid" style="width:97%;"></div></td>
			    	</tr>
			    	<tr>
			    	...
			    	<tr>
				    	<td class="subject" width="100px"   ><span style="background:url(/images/ico/ico_rec01.gif) no-repeat 0 6px">&nbsp;</span> 등&nbsp&nbsp록&nbsp&nbsp일</td> 
		...
		<div id="lst-add-cnslr-window-outer" style="width:320px;float:left;margin-left:10px;">
			<table class="tabular" id="mgmtcg" style="width: 320px;">
			 <tr>
				...
				<tr>
					<td style="width: 100px; height: 30px"><label class="right inline required">▶역량군명</label></td>
	...
	<div id="excel-upload-window" style="display: none; width: 350px;">
        <form method="post" action="<%= architecture.ee.web.util.ServletUtils.getContextPath(request) %>/admin/ca/ca_cmpt_excel_upload.do?">
            <!-- ※ 템플릿을 다운 받아 역량정보를 작성 후 업로드 하세요<br> -->
            ※ 역량군 코드는 아래 내용을 참고하여 작성하세요.<br>
            ...
            <div>
                ...
                <div id="cmpgroupRefTreeview"   style="height: 200px; "></div>
            </div>
        </form>
    </div>
